...
class AdminSensorsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AdminSensorsScreen> createState() => _S();
}
...
class _S extends ConsumerState<AdminSensorsScreen> {
      ...
      body: Column(children: [
        ...
        Expanded(
          ...
          data: (list) {
            final filtered = list
                .where((s) =>
                    s.name.toLowerCase().contains(_q) ||
                    s.location.toLowerCase().contains(_q))
                .toList();
            return RefreshIndicator(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (_, i) {
                  final s = filtered[i];
                  final r = readings.firstWhere((r) => r.sensorId == s.id,
                      orElse: () => Reading(
                          id: "",
                          sensorId: "",
                          temperature: 0,
                          ...
                          recordedAt: ""));
                  final hasAlert =
                      alerts.any((a) => a.sensorId == s.id && !a.acknowledged);
                  return Container(
                      ...
                      child: Column(
                          ...
                          children: [
                            ...
                            if (r.sensorId.isNotEmpty)
                              Padding(
                                  padding: const EdgeInsets.only(top: 4),
                                  child: Text(
                                      'Temp: ${r.temperature.toStringAsFixed(1)}°C · Hum: ${r.humidity.toStringAsFixed(0)}%',
                                      style: const TextStyle(
                                          fontSize: 10,
                            ...
                            Row(children: [
                              Expanded(
                                  child: _Btn(
                                      ...
                                      const Color(0xFF6B7280),
                                      () => context
                                          .go('/admin/sensor/${s.id}/edit'))),
                              const SizedBox(width: 6),
                              Expanded(
                                  child: _Btn(
                                      'Access', Icons.people, purple, () {})),
                              const SizedBox(width: 6),
                              Expanded(
                                  child: _Btn(
                                      'Delete',
  ...
  Widget _Btn(String l, IconData ic, Color c, VoidCallback fn) =>
      GestureDetector(
          onTap: fn,
          child: Container(
              ...
              child:
                ...
                Text(l,
                    style: TextStyle(
                        fontSize: 9.5, color: c, fontWeight: FontWeight.w500))
              ])));
        ...
        builder: (_) => AlertDialog(
                ...
                actions: [
                  ...
                  TextButton(
                      onPressed: () async {
                        Navigator.pop(ctx);
                        await ref.read(apiProvider).deleteSensor(id);
                        ref.invalidate(sensorsProvider);
                      },

...
import {
  Leaf, LayoutDashboard, Radio, Map, Bell, UserCircle, Download,
  LogOut, Sun, Moon, Menu, X, WifiOff, CheckCheck
} from 'lucide-react'

const navItems = [
  { to: '/dashboard',         icon: LayoutDashboard, label: 'Dashboard',  end: true },
  { to: '/dashboard/sensors', icon: Radio,            label: 'My Sensors' },
  { to: '/dashboard/map',     icon: Map,              label: 'Sensor Map' },
        ...
        <div className={`${bgColor} px-6 py-5`}>
          ...
          <div className="mt-3 text-center">
            ...
            <div className="w-full bg-black/10 rounded-full h-1 mt-1">
              <div className={`h-1 rounded-full ${barColor} transition-all duration-1000`} style={{width:(countdown/30*100)+'%'}}/>
            </div>
          </div>
        </div>
      </div>
    ...
    <>
      <div className={`flex items-center gap-3 px-5 py-5 border-b ${dark ? 'border-gray-800' : 'border-gray-100'}`}>
        ...
        <div>
          ...
          <div className={`text-xs ${dark ? 'text-gray-500' : 'text-gray-400'}`}>My Dashboard</div>
        </div>
      </div>
          ...
          <NavLink key={to} to={to} end={end} onClick={() => setMobileOpen(false)}
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                isActive
                  ? dark ? 'bg-teal-500/15 text-teal-400' : 'bg-teal-50 text-teal-700'
      ...
      <div className={`mx-3 mb-3 px-3 py-2.5 rounded-xl flex items-center gap-2.5 ${dark ? 'bg-gray-800' : 'bg-gray-50'}`}>
        {connected
          ? <><span className="w-2 h-2 bg-teal-400 rounded-full animate-pulse shrink-0"/><span className={`text-xs ${dark ? 'text-gray-400' : 'text-gray-500'}`}>Live feed active</span></>
          : <><WifiOff size={13} className="text-red-400 shrink-0"/><span className={`text-xs ${dark ? 'text-gray-400' : 'text-gray-500'}`}>Reconnecting...</span></>}
      </div>

      <div className={`px-3 pb-4 border-t pt-3 ${dark ? 'border-gray-800' : 'border-gray-100'}`}>
        <button onClick={toggle} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium mb-1 transition-all ${dark ? 'text-gray-400 hover:bg-gray-800 hover:text-gray-200' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'}`}>
        ...
        <div className={`flex items-center gap-3 px-3 py-2 mb-1 rounded-xl ${dark ? 'bg-gray-800' : 'bg-gray-50'}`}>
          ...
          <div className="flex-1 min-w-0">
            <div className={`text-xs font-medium truncate ${dark ? 'text-gray-200' : 'text-gray-800'}`}>{user?.email}</div>
            <div className={`text-xs capitalize ${dark ? 'text-gray-500' : 'text-gray-400'}`}>{user?.role}</div>
          </div>
        </div>
        <button onClick={handleLogout} className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm transition-all ${dark ? 'text-gray-500 hover:bg-gray-800 hover:text-red-400' : 'text-gray-400 hover:bg-red-50 hover:text-red-600'}`}>

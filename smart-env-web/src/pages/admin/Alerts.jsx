...
function RuleModal({ onClose }) {
  ...
  const inputClass = `w-full border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 ${
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className={`w-full max-w-md rounded-2xl shadow-2xl border ${dark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-3">
          <div>
            <label className={`block text-xs font-semibold uppercase tracking-wide mb-1 ${dark ? 'text-gray-400' : 'text-gray-500'}`}>Sensor</label>
          ...
          <div>
            <label className={`block text-xs font-semibold uppercase tracking-wide mb-1 ${dark ? 'text-gray-400' : 'text-gray-500'}`}>Alert type</label>
          ...
          <div>
            <label className={`block text-xs font-semibold uppercase tracking-wide mb-1 ${dark ? 'text-gray-400' : 'text-gray-500'}`}>Threshold value</label>
          ...
          <div>
            <label className={`block text-xs font-semibold uppercase tracking-wide mb-1 ${dark ? 'text-gray-400' : 'text-gray-500'}`}>
          ...
          <div>
            <label className={`block text-xs font-semibold uppercase tracking-wide mb-1.5 ${dark ? 'text-gray-400' : 'text-gray-500'}`}>Trigger source</label>
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose}
              className={`flex-1 border rounded-xl py-2 text-sm font-medium ${dark ? 'border-gray-700 text-gray-300 hover:bg-gray-800' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
              Cancel
            </button>
            <button type="submit" disabled={saving}
              className="flex-1 bg-teal-600 hover:bg-teal-700 text-white rounded-xl py-2 text-sm font-bold disabled:opacity-50">
    ...
    <div className={`p-3 rounded-xl border transition-all ${
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className={`flex items-center gap-1.5 text-xs mb-1.5 ${dark ? 'text-gray-500' : 'text-gray-400'}`}>
            <MapPin size={11} />
          ...
          <div className="flex items-center gap-3 mb-1.5">
            ...
            <div className={`text-center px-2.5 py-1 rounded-lg ${dark ? 'bg-gray-700' : 'bg-gray-100'}`}>
        ...
        {isUnread && (
          <button onClick={() => onAcknowledge(event.id)}
            className="shrink-0 bg-orange-600 hover:bg-orange-700 text-white text-xs font-bold px-3 py-1.5 rounded-xl transition-colors flex items-center gap-1.5">
...
export default function Alerts() {
  ...
  const inputClass = `border rounded-xl px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 ${
  ...
  return (
    <div className={`min-h-full p-4 ${dark ? 'bg-gray-950' : 'bg-gray-50'}`}>
      ...
      {/* Rules */}
      <div className={`rounded-2xl border shadow-sm mb-4 overflow-hidden ${cardBg}`}>
        <div className={`px-4 py-3 border-b ${dark ? 'border-gray-800' : 'border-gray-100'}`}>
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              {ruleSearch && (
                <button onClick={() => setRuleSearch('')} className="absolute right-3 top-2.5">
        ...
        {isLoading ? (
          <div className={`px-4 py-8 text-center text-sm ${subText}`}>Loading...</div>
        ) : filteredRules?.length > 0 ? (
          <div className={'divide-y ' + divider}>
            {filteredRules.map(rule => {
              const ruleSensor = sensors?.find(s => s.id === rule.sensor_id)
              return (
                <div key={rule.id} className={`px-4 py-3 flex items-center gap-4 transition-colors ${dark ? 'hover:bg-gray-800/50' : 'hover:bg-gray-50'}`}>
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-lg ${dark ? 'bg-teal-500/10' : 'bg-teal-50'}`}>
          ...
          <div className={`px-4 py-8 text-center ${subText} text-sm`}>
      ...
      <div className={`rounded-2xl border shadow-sm overflow-hidden ${cardBg}`}>
        <div className={`px-4 py-3 border-b flex items-center gap-3 flex-wrap ${dark ? 'border-gray-800' : 'border-gray-100'}`}>
        ...
        <div className="p-3 space-y-2">
          {filteredEvents?.length > 0 ? (
            ...
            <div className={`py-8 text-center ${subText} text-sm`}>

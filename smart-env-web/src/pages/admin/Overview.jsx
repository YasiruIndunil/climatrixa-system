...
function StatCard({ icon: Icon, label, value, sub, color, dark }) {
  ...
  return (
    <div className={`rounded-2xl p-3 border ${dark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'} shadow-sm`}>
      <div className="flex items-center justify-between mb-1.5">
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center border shrink-0 ${colors[color]}`}>
          <Icon size={16} />
        </div>
        <div className={`text-2xl font-bold ${dark ? 'text-white' : 'text-gray-900'}`}>{value ?? '—'}</div>
      </div>
      <div className={`text-xs ${dark ? 'text-gray-500' : 'text-gray-400'}`}>{label}</div>
      {sub && <div className={`text-[11px] mt-0.5 ${dark ? 'text-gray-600' : 'text-gray-300'}`}>{sub}</div>}
    </div>
  )
}
...
function MetricPill({ label, value, unit, color, dark, predicted }) {
  ...
  return (
    <div className={`rounded-xl px-2.5 py-2 border ${colors[color]}`}>
...
export default function Overview() {
  ...
  const forecastQueries = useQueries({
    queries: activeSensorIds.map(id => ({
      ...
      queryFn: () => api.get(`/ai/forecast/${id}`, { timeout: 60000 }).then(r => r.data),
      retry: false,
      staleTime: 5 * 60 * 1000,
    }))
  })
      ...
      <div className={`rounded-2xl border mb-4 overflow-hidden ${dark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'} shadow-sm`}>
        ...
        <div className={`divide-y ${dark ? 'divide-gray-800' : 'divide-gray-50'}`}>
          {readingsLoading ? (
            ...
            <div className={`px-4 py-8 text-center ${dark ? 'text-gray-600' : 'text-gray-300'} text-sm`}>
      ...
      <div className={`rounded-2xl border overflow-hidden ${dark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'} shadow-sm`}>
        ...
        <div className={`divide-y ${dark ? 'divide-gray-800' : 'divide-gray-50'}`}>
          {alerts?.slice(0, 5).map(a => (
            <div key={a.id} className={`px-4 py-2.5 flex items-start gap-3 ${!a.acknowledged ? (dark ? 'bg-orange-500/5' : 'bg-orange-50') : ''}`}>
            ...
            <div className={`px-4 py-6 text-center text-sm ${dark ? 'text-gray-600' : 'text-gray-300'}`}>No alerts yet</div>
          )}
        </div>
      </div>
    </div>
